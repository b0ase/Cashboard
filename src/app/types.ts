...
export interface PeopleViewProps {
  people: HandCashHandle[]
  onAddPerson: (person: Omit<HandCashHandle, 'id'>) => void
  onUpdatePerson: (id: string, updates: Partial<HandCashHandle>) => void
  selectedOrganization: string | null
}
...
export interface FinancialInstrument {
  id: string
  name: string
  type: 'token' | 'nft_collection' | 'asset_class' | 'stablecoin' | 'utility_token' | 'security_token' | 'governance_token' | 'social_token' | 'other'
  symbol: string
  description: string
  totalSupply: number
  circulatingSupply: number
  maxSupply?: number
  issuer: string // Organization ID
  issuanceDate: string
  contractAddress?: string
  blockchain: 'bsv' | 'ethereum' | 'polygon' | 'solana' | 'other'
  ...
  relatedWallets?: string[] // Wallet IDs
  createdAt: string
  updatedAt: string
}
...
export interface SecurityProduct {
  id: string
  name: 'API Keys' | 'SSH Keys' | 'MCP Servers'
  description: string
  component: React.ReactNode
  icon: React.ReactNode
}
export type BusinessType = Extract<
    WorkflowNode['type'],
    | 'organization'
    | 'role'
    | 'member'
...
export type BusinessItem = { id: string; name: string; description?: string }
